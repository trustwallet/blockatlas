...
func TestParser_add(t *testing.T) {
	...
	batch.fillBatch(txs, context.TODO())
	assert.Equal(t, 8, len(batch.Txs))

	batch.fillBatch(nil, context.TODO())
	assert.Equal(t, 8, len(batch.Txs))
}

func TestParser_getBlockByNumberWithRetry(t *testing.T) {
	block, err := getBlockByNumberWithRetry(3, time.Millisecond*1, getBlock, 1, "", context.TODO())
	if err != nil {
		t.Error(err)
	}
...
func TestParser_getBlockByNumberWithRetry_Error(t *testing.T) {
	now := time.Now()
	block, err := getBlockByNumberWithRetry(2, time.Millisecond*2, getBlock, 0, "", context.TODO())
...
func TestGetTxBatches(t *testing.T) {
	...
	batches := getTxsBatches(txs, 1000, context.TODO())
	assert.Len(t, batches, 10)
	batches = getTxsBatches(txs, 100, context.TODO())
	assert.Len(t, batches, 100)
	batches = getTxsBatches(txs, 500, context.TODO())
	...
	batches = getTxsBatches(txs, 10000, context.TODO())
	...
	batches = getTxsBatches(nil, 100, context.TODO())
	assert.Len(t, batches, 0)

	txs = make(blockatlas.Txs, 1000000)
	batches = getTxsBatches(txs, 5000, context.TODO())
