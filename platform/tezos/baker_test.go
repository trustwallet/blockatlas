...
func TestNormalizeStakeValidators(t *testing.T) {
	tests := []struct {
		name       string
		bakers     []Baker
		validators blockatlas.StakeValidators
	}{
		{
			name: "Test normalize bakers",
			bakers: []Baker{
				{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if gotValidators := NormalizeStakeValidators(tt.bakers); !reflect.DeepEqual(gotValidators, tt.validators) {
				t.Errorf("NormalizeStakeValidators() = %v, want %v", gotValidators, tt.validators)
			}
		})
	}
}
