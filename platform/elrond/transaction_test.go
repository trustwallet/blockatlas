...
var (
	...
	txNegativeValue, _ = mock.JsonStringFromFilePath("mocks/tx_negative_value.json")
...
func TestNormalizeTxs(t *testing.T) {
	var tx1, tx2, tx3, txNegative Transaction

	_ = json.Unmarshal([]byte(txTransferSrc1), &tx1)
	_ = json.Unmarshal([]byte(txTransferSrc1), &tx2)
	_ = json.Unmarshal([]byte(txTransferSrc1), &tx3)
	_ = json.Unmarshal([]byte(txNegativeValue), &txNegative)

	txs := []Transaction{tx1, tx2, tx3, txNegative}
