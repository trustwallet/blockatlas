...
var (
	...
	revertedTx, _      = mock.JsonFromFilePathToString("mocks/reverted_tx.json")
	revertedReceipt, _ = mock.JsonFromFilePathToString("mocks/reverted_receipt.json")
)
...
func TestNormalizeTransaction(t *testing.T) {
	tests := []struct {
		...
		{"Test normalize VET transfer transaction", "0xb5e883349e68ab59307d1604555ac890fac47128", transferSrc, trxId, blockatlas.Tx{
...
func TestNormalizeTokenTransaction(t *testing.T) {
	tests := []struct {
		...
		{"Normalize VIP180 token transfer", transferLogSrc, trxReceipt, blockatlas.TxPage{
			{
		...
		{"Normalize reverted token transfer", revertedTx, revertedReceipt, blockatlas.TxPage{
			{
				...
				Date:   1610326580,
				Type:   blockatlas.TxTokenTransfer,
				Fee:    blockatlas.Amount("82618000000000000000"),
