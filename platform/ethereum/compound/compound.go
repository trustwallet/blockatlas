...
func (p *Provider) GetAccountLendingContracts(req blockatlas.AccountRequest) ([]blockatlas.AccountLendingContracts, error) {
	...
	contracts := make([]blockatlas.AccountLendingContracts, 0)
	for _, acc := range accounts {
		accountContracts := blockatlas.AccountLendingContracts{Address: acc.Address, Contracts: make([]blockatlas.LendingContract, 0)}
		for _, t := range acc.Tokens {
			...
			accountContracts.Contracts = append(accountContracts.Contracts, blockatlas.LendingContract{
				Asset:         assetInfo,
				CurrentAmount: strconv.FormatFloat(asFloat(t.SupplyBalanceUnderlying.Value), 'f', 10, 64),
			})
		}
		sort.Slice(accountContracts.Contracts, func(a, b int) bool {
			return accountContracts.Contracts[a].CurrentAmount > accountContracts.Contracts[b].CurrentAmount
		})
		contracts = append(contracts, accountContracts)
	}
	sort.Slice(contracts, func(a, b int) bool { return contracts[a].Address > contracts[b].Address })
