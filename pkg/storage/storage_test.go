...
type cacheFactory func(*testing.T, time.Duration) cache.Storage
...
func typicalGetSet(t *testing.T, newCache cacheFactory) {
	...
	value := "foo"
	if err = Set("value", value, DEFAULT); err != nil {
...
func incrDecr(t *testing.T, newCache cacheFactory) {
	...
	if err = Set("int", 10, DEFAULT); err != nil {
...
func testReplace(t *testing.T, newCache cacheFactory) {
	...
	if err = Replace("notexist", 1, FOREVER); err != cache.ErrNotStored && err != ErrCacheMiss {
		t.Errorf("Replace in empty cache: expected ErrNotStored or ErrCacheMiss, got: %s", err)
	}

	// Set a value of 1, and replace it with 2
	if err = Set("int", 1, time.Second); err != nil {
		t.Errorf("Unexpected error: %s", err)
	}

	if err = Replace("int", 2, time.Second); err != nil {
		t.Errorf("Unexpected error: %s", err)
	}
	var i int
	if err = Get("int", &i); err != nil {
	...
	time.Sleep(2 * time.Second)
	if err = Replace("int", 3, time.Second); err != cache.ErrNotStored && err != ErrCacheMiss {
		t.Errorf("Expected ErrNotStored or ErrCacheMiss, got: %s", err)
	}
	if err = Get("int", &i); err != ErrCacheMiss {
